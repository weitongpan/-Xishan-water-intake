...
export default {
    methods: {
        ...
        // vue版本的组件进场处理
         vueEnter() {
            // 动画进入时的类名
            const classs = getclasss(this.mode)
            // 定义状态和发出动画进入前事件
            this.status = 'enter'
            this.$emit('beforeEnter')
            this.inited = true
            this.display = true
            this.classes = classs.enter
            this.$nextTick(async () => {
				// #ifdef H5
